...
def has_prefix(s, prefix):
    ...
    if prefix == empty: return True
            ...
            return True if prefix[1] == empty else has_prefix(s[1], prefix[1])
    else: return False
...
def has_sublist(s, sublist):
    ...
    elif (s == empty) or (len(s) < len(sublist)): return False
    else:
        if s[0]==sublist[0]:
            if sublist[1] == empty:
                return True
            else:
                if rest(s) == rest(sublist):
                    return has_sublist(rest(s), rest(sublist))
                else:
                    return False
        else:
            return has_sublist(s[1], sublist)
...
def has_kitty_gene(dna):
    ...
    False
    """
    return has_sublist(dna, link('C', link('A', link('T', link('C', link('A', link('T'))))))) 
...
def count_change(amount, denominations):
    ...
    if (denominations == link(1)):
        return 1
    elif (amount <= 0):
        return 0
    else:
        return count_change(amount-denominations[0], denominations) + count_change(amount, denominations[1])
